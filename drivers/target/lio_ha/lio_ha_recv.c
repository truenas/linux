 ...
 *      points to the real nacl.  See ha_recv_session_create().
 *
 * A single system-wide TPG is created at ha_recv_init() following the
 ...
 * 256 buckets (8 bits) -- more than enough for a single HA peer.
 */
...
/*
 * Demo mode: LIO auto-creates a node_acl for any initiator name.
 * Required because STANDBY sessions have arbitrary IQNs that are not
 * configured in the ha_recv TPG's ACL list.
 */
...
{
	/* Do not cache demo-mode ACLs between sessions. */
	return 0;
}
...
{
	/* Nothing to do: STANDBY will time out and retry. */
}
...
/*
 * Decode CMD_FORWARD and dispatch to ha_recv_submit_cmd().
 * Registered with the TCP layer in ha_recv_init().
 */
static void ha_recv_cmd_forward_handler(const void *buf, size_t len)
{
	...
	ha_recv_submit_cmd(be64_to_cpu(fwd->session_id),
...
int ha_recv_session_create(const char *initiator_name, u64 session_id,
			   ...
			   const char *fabric_name)
{
	...
	if (real_nacl) {
		...
		/*
		 * Fallback: demo-mode nacl on ha_recv_tpg.  LUN lookups
		 * will fail until the operator configures the TPG/ACL.
		 */
		if (real_tpg)
			pr_warn_ratelimited("ha_recv: no ACL for %s on %s/%s tpg%u, disabled\n",
					    initiator_name, fabric_name,
					    target_name, tpg_tag);
		else
			pr_warn_ratelimited("ha_recv: TPG %s/%s tpg%u not found, disabled\n",
					    fabric_name, target_name, tpg_tag);

		se_sess = target_setup_session(&ha_recv_tpg.se_tpg,
					       0, 0, TARGET_PROT_NORMAL,
					       initiator_name, e, NULL);
		if (IS_ERR(se_sess)) {
			pr_err("ha_recv: target_setup_session(%s) failed: %ld\n",
			       initiator_name, PTR_ERR(se_sess));
			kfree(e);
...
/*
 * Decode SESSION_CONNECT and call ha_recv_session_create().
 * Registered with the TCP layer in ha_recv_init().
 */
static void ha_recv_session_connect_handler(const void *buf, size_t len)
{
	...
	ha_recv_session_create(initiator_name, session_id,
			       target_name, tpg_tag, fabric_name);
}

/*
 * Decode SESSION_DISCONNECT and call ha_recv_session_destroy().
 * Registered with the TCP layer in ha_recv_init().
 */
static void ha_recv_session_disconnect_handler(const void *buf, size_t len)
{
	...
	ha_recv_session_destroy(be64_to_cpu(msg->session_id));
}

void ha_recv_session_destroy(u64 session_id)
{
	struct ha_recv_sess_entry *e;
...
void ha_recv_data_rx(u64 cookie, const void *data, u32 len)
{
	struct ha_recv_data_entry *de;
...
void ha_recv_submit_cmd(u64 session_id, const void *cdb, size_t cdb_len,
			u64 cookie, u32 lun, u32 data_length, u8 data_dir)
{
	 ...
	 * target TPG (set by ha_recv_session_create), so lun_entry_hlist
...
void ha_recv_exit(void)
{
	 ...
	 *
	 * On STANDBY, ha_recv_sess_ht is always empty (STANDBY never receives
	 * SESSION_CONNECT), so this loop is a no-op during failover rmmod.
	 */
	spin_lock(&ha_recv_sess_lock);
