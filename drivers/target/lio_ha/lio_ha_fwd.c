 ...
 *   3. ACTIVE: DATA -> ha_recv_data_rx() buffers by cookie.
 *      CMD_FORWARD -> ha_recv_submit_cmd() -> target_submit_cmd() ->
 ...
 *
 * WRITE data (S->A): cookie not in ha_fwd_ht -> forward to ha_recv_data_rx()
...
static void ha_fwd_data_handler(u64 cookie, const void *data, u32 data_len)
{
	...
	if (e) {
		 ...
		 */
		ha_recv_data_rx(cookie, data, data_len);
	}
}
