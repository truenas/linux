 ...
 *                  On accept: calls lio_ha_on_connected().
 *
 * ha_connect_task: loop: if disconnected, try kernel_connect with 5 s
 *                  connect timeout; exponential backoff (1 s -> 30 s) on
 *                  failure.  On success: calls lio_ha_on_connected().
 *
 * ha_rx_task     : started per-connection inside lio_ha_on_connected().
 *                  Reads wire header + payload in a loop.  On error:
 *                  calls lio_ha_on_disconnected(), then exits.
 *
 * Locking
 * -------
 * ha_tcp_lock (mutex): protects ha_conn_sock, ha_rx_task, and the
 *     ha_state field in lio_ha_cfg.
 *
 * ha_send_lock (mutex): serialises concurrent callers of
 *     lio_ha_tcp_send() so that header and payload are sent as a unit.
 *     lio_ha_on_disconnected() acquires ha_send_lock before calling
...
/*
 * Maximum CTL-channel payload (header inclusive).  DATA-channel
 * payloads are raw SCSI data and are handled without this limit
 * (see ha_rx_task's DATA channel path).
 */
/* LIO_HA_MAX_CTL_PAYLOAD is defined in lio_ha_tcp.h */

/*
 * Connect-loop timing.  Both values are in milliseconds.
 */
...
static int  lio_ha_rx_fn(void *arg);
static void lio_ha_on_disconnected(struct socket *sock);
 ...
 */
static void lio_ha_on_connected(struct socket *sock)
{
	...
	rx = kthread_run(lio_ha_rx_fn, sock, "lio_ha_rx");
	if (IS_ERR(rx)) {
...
static int lio_ha_listen_fn(void *unused)
{
...
static int lio_ha_connect_fn(void *unused)
{
...
int lio_ha_tcp_send(enum lio_ha_channel chan, const void *payload, u32 len)
{
	 ...
	 * until ha_send_lock is released (see lio_ha_on_disconnected).
...
int lio_ha_tcp_init(void)
{
	...
	ha_listen_task = kthread_run(lio_ha_listen_fn, NULL, "lio_ha_listen");
	...
	ha_connect_task = kthread_run(lio_ha_connect_fn, NULL, "lio_ha_connect");
...
void lio_ha_tcp_exit(void)
{
	 ...
	 * unblocks its kernel_recvmsg; it calls lio_ha_on_disconnected()
