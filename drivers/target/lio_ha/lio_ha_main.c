 ...
 * lio_ha_pr_apply(), lio_ha_failover_device(), and the forward_active handler.
 *
...
/* lio_ha_failover_device() snapshots this table and calls             */
...
/* Work item queued by lio_ha_pr_change() to send PERS_ACTION via TCP. */
...
static void lio_ha_pr_table_flush(void)
{
...
static void lio_ha_pr_work_fn(struct work_struct *work)
{
...
static void lio_ha_session_create(struct se_session *sess)
{
...
static void lio_ha_session_destroy(struct se_session *sess)
{
...
static const struct lio_ha_ops ha_ops = {
	...
	.session_create  = lio_ha_session_create,
	.session_destroy = lio_ha_session_destroy,
};
...
static void lio_ha_pr_change(struct se_device *dev, u8 action,
...
/* then sends LIO_HA_MSG_LUN_SYNC_DONE to advance STANDBY's ha_state */
/* from CONNECTED to SYNCED.                                           */
/* ------------------------------------------------------------------ */
 ...
 */
static int lio_ha_send_lun_sync_for_device(struct se_device *dev, void *data)
{
 ...
 * Only ACTIVE (forward_active == 0) sends LUN_SYNC / LUN_SYNC_DONE.
 */
static void lio_ha_on_connect(void)
{
	struct lio_ha_msg_lun_sync_done done = {};

	/* Only ACTIVE sends LUN_SYNC. */
	if (atomic_read(&lio_ha_forward_active))
		return;

	target_for_each_device(lio_ha_send_lun_sync_for_device, NULL);
...
static void failover_add_reg_cb(struct se_node_acl *nacl,
				struct se_lun *lun,
				u64 mapped_lun,
				void *data)
{
 ...
 * the backend.
 */
static int lio_ha_failover_device(struct se_device *dev, void *data)
{
	char dev_name[LIO_HA_DEV_NAME_LEN];
...
/* ha_state (read-only) -- middleware polls until "synced" before writing
 * NONOPTIMIZED on STANDBY; "disconnected" on ACTIVE triggers an alert.
 */
 ...
 *   lio_ha_failover_device() for each se_device -- see that function for
					   ...
					   const char *page, size_t count)
{
	...
	if (val == 1) {
		 ...
		 *
		 * target_for_each_device() calls lio_ha_failover_device() for
		 ...
		 *
		 * The flag is cleared BEFORE lio_ha_failover_device() calls
		...
		target_for_each_device(lio_ha_failover_device, NULL);
...
static int __init lio_ha_init(void)
{
	...
	lio_ha_tcp_register_connect_handler(lio_ha_on_connect);
