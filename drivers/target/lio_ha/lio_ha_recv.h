 ...
 *
 * ha_recv_init() / ha_recv_exit() are called from lio_ha_main.c.
 *
 * ha_recv_session_create() and ha_recv_session_destroy() are called
 * by the TCP channel layer when SESSION_CONNECT and SESSION_DISCONNECT
 * messages arrive.
 *
 * ha_recv_submit_cmd() is called by the TCP channel layer when a
 ...
 *
 * Allocated by ha_recv_submit_cmd(); freed in ha_recv_release_cmd()
...
/**
 * ha_recv_session_create - create a synthetic se_session for a STANDBY session
 ...
 * correct LUN mappings.  Falls back to a demo-mode ha_recv nacl (with no
 * LUN mappings) if the TPG or ACL cannot be found.
 *
...
/**
 * ha_recv_session_destroy - tear down the synthetic se_session
 * @session_id: same opaque u64 passed to ha_recv_session_create()
 *
...
/**
 * ha_recv_data_rx - receive WRITE data from STANDBY on ACTIVE
...
/**
 * ha_recv_submit_cmd - submit a forwarded command to LIO core on ACTIVE
