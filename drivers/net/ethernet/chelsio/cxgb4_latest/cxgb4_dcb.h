...
#if !defined(CONFIG_DCB) && !defined(CONFIG_DCB_MODULE)
#error Cannot build cxgb4 for DCBX support without kernel DCBX configured
#endif
...
#define INIT_PORT_DCB_CMD(__pcmd, __port, __op, __action) \
	...
	(__pcmd).op_to_portid = \
		cpu_to_be32(V_FW_CMD_OP(FW_PORT_CMD) | \
			    F_FW_CMD_REQUEST | \
			    F_FW_CMD_##__op | \
			    V_FW_PORT_CMD_PORTID(__port)); \
	(__pcmd).action_to_len16 = \
		cpu_to_be32(V_FW_PORT_CMD_ACTION(__action) | \
			    FW_LEN16(pcmd)); \
    } while (0)
...
void cxgb4_dcb_reset(struct net_device *);
